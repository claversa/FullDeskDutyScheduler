...
const Form = () => {
    ...
    return (
        <form className='flex flex-col items-center mt-20' onSubmit={handleSubmit(onSubmit)}>
            <h1 className='text-2xl py-5 text-blue-800 font-bold'>Welcome RA!</h1>
                        ...
                        {intervals[day].map((interval, index) => (
                            <div key={index} className='mb-4'>
                                <div className='flex flex-row justify-center'>
                                    ...
                                    <button type="button" className='hover:bg-red-500 ml-4 shadow-lg rounded-lg text-white text-xs px-2 py-1 bg-red-600' onClick={() => removeInterval(day, index)}>Remove</button>
                                </div>
                            </div>
                        ))}
